...
/*
Теперь два метода в IDipParent

List<IDdeID> getDDEChildren();
List<IDipDocumentElement> getDdeElements();
- если вносим изменения - нужно работать с getDDEChildren, из-за этого не работает Down


Проверить переименование!!!
При возвращении в документ - лагает!!!

Везде где возвращается список элементов (не ID)
...
Там где возвращаются элементы - сделать Imutable ?? Возможны баги с глоссарием, переменными и т.п. Возможно они будут добавляться в список
*/
