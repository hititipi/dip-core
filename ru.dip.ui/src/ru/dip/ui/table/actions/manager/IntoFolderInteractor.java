...
import ru.dip.ui.table.dialog.SelectIntoFodlerDialog;
...
public class IntoFolderInteractor {
	...
	public IDipTableElement moveIntoFolder(IDipParent targetDipParent, IDipTableElement sourceElement, IDipDocumentElement sourcedReq,
		...
		LinkInteractor.instance().updateLinks(sourcedReq, DdeStorage.instance.get(targetDipParent.getChild(sourcedReq.name())));
		// table move
		if (sourceElement instanceof TableNode) {
			IDipParent parent = (IDipParent)  DdeStorage.instance.get(targetDipParent.getChild(sourcedReq.name()));
			ITableNode newNode = targetNode.addNewFolderToBegin(parent);
			sourceElement.parent().delete(sourceElement);
			return newNode;
		} else {
			DipUnit unit = DdeStorage.instance.get(targetDipParent.getChild(sourcedReq.name()));
