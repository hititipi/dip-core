 ...
 *
 *******************************************************************************/
package ru.dip.ui.table.actions.manager;
...
public class CreateFileInteractor {
	...
	public void undoCreate(CreateFileResult result) throws CreateResourceException {
		...
		IDipDocumentElement dipDocElement = DdeStorage.instance.get(((IDipParent) parent.get()).getChild(result.getFileName()));
	...
	private class FileCreator {
		...
		private void setWizardLabels(TableNewFileWizard wizard) {
			if (fSelectedDipDocElement instanceof IDipParent) {
				...
				if (fTargetDipParent.isFileNumeration()) {
					IDdeID previous = DipTableUtilities.getPreviousElement(fSelectedDipDocElement);
					String beforeName = DipTableUtilities.getNextNumber(fTargetDipParent, previous.getElement());
	...
	public void undoCreate(CreateFolderResult result) throws CreateResourceException {
		...
		IDipDocumentElement dipDocElement = DdeStorage.instance.get(((IDipParent) parent.get()).getChild(result.getFolderName()));
		if (dipDocElement == null) {
			throw new CreateResourceException(UNDO_CREATE_FOLDER_EXCEPTION);
		}
		try {
	...
	private class FolderCreater {
		...
		void getNewIndex() {
			if (fSelectedDipDocElement == null) {
				fIndex = fBefore ? -1 : fTargetParent.getDDEChildren().size();
			} else {
				fIndex = fBefore ? fSelectedDipDocElement.getIndex() : fSelectedDipDocElement.getIndex() + 1;
			}
		}
		...
		void checkReserved() {
			if (fReserved) {
				IDipElement reserverDipElement = DdeStorage.instance.get(fTargetParent.getChild(fNewFolderName));
