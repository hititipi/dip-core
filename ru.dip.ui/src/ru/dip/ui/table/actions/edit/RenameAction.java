...
import ru.dip.ui.table.actions.undo.CancelledDocumentAction;
...
public class RenameAction extends DocumentAction implements CancelledDocumentAction {
	...
	public void undo() throws Exception {
		...
		IDipDocumentElement dipDocumentElement = DdeStorage.instance.get(((IDipParent)dipParentOpt.get()).getChild(fResult.getNewName()));
		if (dipDocumentElement == null) {
			throw new RenameDIPException("Undo Rename Error");
		}
		
		fTableComposite.doRenameWithoutResult(dipDocumentElement, fResult.getOldName(), fResult.getNewName(),  false);		
	}
	...
	public void redo() throws Exception {
		...
		IDipDocumentElement dipDocElement = DdeStorage.instance.get(((IDipParent)dipParentOpt.get()).getChild(fResult.getOldName()));
		if (dipDocElement == null) {
			throw new RenameDIPException("Redo Rename Error");
		}		
		fTableComposite.doRenameWithoutResult(dipDocElement, fResult.getNewName(), fResult.getOldName(), false);
	}
